  ...
  return (
    <section className="flex min-h-[100svh] items-center bg-brand-dark px-5 py-16 text-white md:min-h-0 md:py-28">
      <div className="mx-auto w-full max-w-[900px] text-center">
        <div>
          <p className="section-eyebrow mb-6 inline-flex rounded-full border border-blue-300/30 bg-blue-400/10 px-4 py-2 font-bold text-blue-100">제{SESSION.round}차 · 실시간 무료특강 · 참가비 0원</p>
          ...
          <div className="hero-meta mt-8 grid gap-3 font-semibold text-slate-200 md:grid-cols-3">
            <p className="rounded-lg bg-white/5 px-3 py-3">📅 {SESSION.fullDateLabel}<br />{SESSION.timeRangeLabel}</p>
...
export function TrustBar() {
  const items = [
    ...
    { number: `${SESSION.round}차`, description: "누적 무료특강", full: `누적 ${SESSION.round}차 무료특강` },
